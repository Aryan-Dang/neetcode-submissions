...
class Solution:
    def countComponents(self, n: int, edges: List[List[int]]) -> int:
        ...
        for edge in edges:
            ...
            #especially when we're relying on checking from 0 -> n then we would need to check if the larger node is in the graph so we need it in the graph to indicate that there's an edge 'from' this node...
            if x not in graph:
                graph[x] = [y]
            else:
                graph[x].append(y)
